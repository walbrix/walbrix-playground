...
/usr/bin/mysql -u root -e "create database orangehrm" || exit 1
/usr/bin/mysql -u root -e 'grant all privileges on orangehrm.* to orangehrm@localhost' || exit 1
/usr/bin/mysql -u root orangehrm < /tmp/initial_database.sql
