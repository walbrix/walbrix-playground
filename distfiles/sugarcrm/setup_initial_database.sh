...
/usr/bin/mysql -u root -e 'create database `sugarcrm`' || exit 1
/usr/bin/mysql -u root -e 'grant all privileges on `sugarcrm`.* to '\''sugarcrm'\''@localhost' || exit 1
/usr/bin/mysql -u sugarcrm sugarcrm < /initial_database.sql
/usr/bin/mysql -u sugarcrm sugarcrm <<EOF
select 1
EOF
