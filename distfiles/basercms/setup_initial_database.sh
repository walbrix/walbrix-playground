...
/usr/bin/mysql -u root -e "create database basercms" || exit 1
/usr/bin/mysql -u root -e 'grant all privileges on basercms.* to basercms@localhost' || exit 1
/bin/gunzip -c /initial_database.sql.gz | /usr/bin/mysql -u root basercms || exit 1
