...
/usr/bin/mysql -u root -e "create database zabbix" || exit 1
/usr/bin/mysql -u root -e 'grant all privileges on zabbix.* to zabbix@localhost' || exit 1
/usr/bin/mysql -u zabbix zabbix < initial-database.sql
/usr/bin/mysql -u zabbix zabbix < initial-database-lang.sql
