...
/usr/bin/mysql -u root -e "create database phplist" || exit 1
/usr/bin/mysql -u root -e 'grant all privileges on phplist.* to phplist@localhost' || exit 1
if [ -f /tmp/initial-database.sql ]; then
	/usr/bin/mysql -u phplist phplist < /tmp/initial-database.sql
fi
